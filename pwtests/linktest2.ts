...
(async () => {
  ...
  // Click the button
  await page.click('text=Eikö sinulla ole tiliä? Rekisteröidy');
  ...
  // Verify text content
  if (textContent && textContent.includes('Rekisteröidy')) {
    console.log('Register link Test Passed!');
  } else {
  ...
  console.log(`The load time for the register page is ${loadTime2} milliseconds.`);
  await page.waitForTimeout(2000); // Wait for 2 seconds
  // Close the browser
  await browser.close();
})();